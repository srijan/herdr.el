...
- `ssh -L local_socket:remote_socket` carries the herdr protocol unchanged. **Measured in U1** on 2026-09-14, a macOS client against a Fedora server running herdr 0.9.0: `ping` answered protocol 22, `session.snapshot` returned the remote's session, and an `events.subscribe` held open across a 180-second idle still delivered an event made afterwards. `herdr-rpc-call` needed no change. `docs/protocol.md` records the failure shapes with it.
- **The forward has a host-policy precondition the tunnel decision has to carry.** On a Fedora server SELinux labels the socket under `~/.config/herdr` as `config_home_t` and denies `sshd-session` access to it, so every connection through the tunnel fails while the server is perfectly healthy. `chcon -t user_tmp_t <socket>` fixes it without privilege but does not survive the socket being recreated. U5 has to decide what herdr.el does about this, and the answer is not "assume it works": the client-visible symptom is `empty_response`, which is the same symptom as a stopped server.
...
- **There is no remote terminal-attach command, so KTD4 has no herdr-native alternative.** `herdr terminal attach <TERMINAL_ID>` is the command a pane buffer runs and it takes one option, `--takeover`; it speaks to the local server's socket and has no remote flag. The `herdr --remote <ssh-target>` that sits beside it in `herdr --help` is an *app* attach - its own `--remote-keybindings` option calls it that - and it runs a herdr client on the far host over SSH rather than forwarding anything. Measured on 2026-09-14: it reached the remote and reported its version, then failed with `nested herdr is disabled by default` from the remote client, and it left no local socket and no standing SSH process behind. It is the TUI equivalent of `ssh -t <host> herdr`, useful to a person and unusable to a pane buffer. TRAMP to `herdr terminal attach` on the far host stays the only route for R2.
...
- A tunnel to a path with no listener and a tunnel to a host with no herdr produce the same reported state *through the forward*, because nothing observable separates them there. The test says so, so that nobody re-invents the distinction at that layer. **Corrected while building U5:** the two are separated earlier and by something observable. `herdr session list --json` exists and reports each session's `socket_path`, so the remote path is resolved by running herdr on the far host — which answers "is herdr installed there" as a side effect, before any forward is built. A host with no herdr fails at that step with `ssh_failed` and ssh's own message; only a host that has herdr and no running server reaches `no_answer`.
...
- **A remote schema means running the remote binary.** The local fetch is a bounded subprocess; the remote one is that over TRAMP, with a slower and less predictable failure surface. A connection whose schema cannot be fetched must still be usable for everything the curated commands cover, because those are hand-written and do not need the schema at all - only `herdr-call` and the drift tests do.
