...
- **herdr 0.9.0 attaches to a remote server itself**, which U1 found while confirming how a session is named: `herdr --remote <ssh-target> [--session <name>]`, with `--remote-keybindings <local|server>`. KTD4 sends remote terminals through TRAMP to `herdr terminal attach` on the far host; this is a second route to the same place, and U5 should say which it uses and why rather than discovering the choice late. It does not affect the control plane, which is what the tunnel carries.
