...
- Making the repair asynchronous. This unit's extracted pair is synchronous, which is right while there is one server: the settle realigns connection B against the pane set the repair just settled, and synchronous ordering is what gives it that for free. It stops being right the moment a second connection exists, because one silent server would then stall every other connection's repair in turn on Emacs's single thread. The connection plan's U3 owns the conversion and the continuation that replaces the ordering. Build the pair here so that conversion has one function to change, not three call sites.
...
- `herdr-state-reconcile-workspaces` has two callers today: `herdr-state--settle`, which the 0.9.0 upgrade added, and the directory poll. The settle's call is the one step 1 extracts and keeps; the poll's is the one this unit moves. No third path reads it.
...
- `test/herdr-term-test.el` - move the coupling test, keep the directory-sync tests, and rewrite the two timer-teardown tests named in step 4.
...
2. Add a repeating repair timer to `herdr-state`, started by `herdr-state-start` and cancelled by `herdr-state-stop` alongside the reconnect, resubscribe and settle timers. Its callback is the extracted pair. The re-entrancy guard belongs to that function and covers every entry path into it, not to the timer: `herdr-rpc-call` services due timers while it waits, which is why `herdr-term--poll-in-progress` exists at all, and that flag guards the poll against itself and against nothing else. A poll firing inside the settle's `pane.list` wait already nests two deep today. Say what a caller that finds the guard taken does - the timer skips its tick, the settle must not, because its realignment of connection B depends on the repair having happened.
...
**Execution note on the startup rollback:** `herdr-state-start`'s error handler clears `herdr-state--running` and removes one hook. It closes no process and cancels no timer, and that is already a leak - not one this unit introduces. `herdr-state-start` announces `resync` before it opens the streams, so the listener it just added can arm the resubscribe timer; and `herdr-state--open-streams` opens connection A before attempting B, so a B that signals leaves A open. A start that fails past that point today leaves an open global stream and an armed resubscribe timer behind a `herdr-state--running` of nil. Adding a repair timer makes a third. So the rollback this unit writes releases everything the attempt acquired, not just the new timer: close whichever streams opened, cancel whichever timers armed, and leave the generation bumped so a late reply is dropped. Assert the unwind with a failure injected before the repair timer is armed and again after, because those are different leaks and only one of them is new.
...
- `herdr-term-teardown` still cancels a pending directory debounce, and nils its handle. This is the timer whose only canceller the poll's teardown used to be.
...
- The guard holds across callers, not just against a repeated tick: a repair entered while another is in flight does not nest. Drive it from two different entry paths, since one calling itself is the case that already worked.
...
| Unit | Done when |
|---|---|
| U1 | The cache arms and cancels its own repair timer under `herdr-state-repair-interval`, `herdr-term` reaches repair only through the published entry point, `herdr-term-directory-interval` is gone, and both documents describe the cadence as the cache's. |
| U2 | `herdr-workspace.el` compiles, requires no herdr module, and answers every accessor for a record carrying only an id. |
| U3 | The enforcement test reports no offender, the seven reads it cannot see are migrated too, and no surface renders an empty-labelled workspace as nameless: the dashboard heading shows its id, an id-shaped picker row carries that id exactly once, a path-shaped place row keeps its path, and the confirmations are unchanged. |
