...
- `docs/configuration.md`, `docs/architecture.md`, `docs/troubleshooting.md` - correct the cadence and the remedy.

**Approach:**

1. Add a repeating repair timer to `herdr-state`, started by `herdr-state-start` and cancelled by `herdr-state-stop` alongside the reconnect, resubscribe and settle timers. Its callback reconciles panes, then workspaces, under the same re-entrancy guard and the same `herdr-rpc-background-timeout` binding the poll uses today.
2. Add `herdr-state-repair-interval`, defaulting to 5.0 seconds, with nil meaning no periodic repair. The prefix follows `herdr-state-settle-delay`, which already names cache timing, and the default matches the cadence the directory poll runs at today so nobody's repair rate changes. Document it under the event stream, not under terminals, and point the removed directory-interval row at it.
3. Reduce `herdr-term` to a listener. `herdr-term--on-state-change` keeps syncing buffer directories, gated on `herdr-term-track-directory`. Delete the poll, the repeating timer and its start and stop. Delete `herdr-term-directory-interval` with them, and its row in `docs/configuration.md`: the timer is its only reader, and the cache's repair interval replaces it.
4. Publish a repair-now entry point alongside the cadence: the repair timer's own callback, callable on demand, bound to the background timeout like every other timer-borne call. `herdr-term-directory-debounce` keeps its meaning and its cancel-and-rearm shape, but the debounce calls the repair entry point instead of reaching for a reconcile function. Without this the debounce would re-read a cache nothing refreshed: a `cd` produces no event herdr.el acts on - `layout.updated` is a no-op in the reducer - so a pane's directory reaches the cache only through a pane reconcile, and directory tracking would silently degrade from the debounce interval to the repair interval.
5. Correct the three documents. `docs/configuration.md` currently tells the reader to decrease `herdr-term-directory-interval` when dead panes linger, and claims the poll runs only while terminal buffers exist. `docs/troubleshooting.md` repeats the first. `docs/architecture.md` states the cadence as every directory poll.
...
- **Removing the terminal poll may leave directories stale.** A `cd` produces no event, so directory freshness rides on the repair cadence. U1 step 4 makes this an explicit decision rather than a discovery.
