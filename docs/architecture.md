...
| `herdr-state--reconcile-workspaces-async` | `workspace.list` | The workspace set. |
...
`herdr-tree.el` is pure. It requires `herdr-state.el`, `herdr-pane.el` and `herdr-workspace.el` only.
`herdr-tree-build` takes a state and
